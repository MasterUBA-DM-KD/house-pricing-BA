...
def get_suburb(lat: float, lon: float) -> str:
    location = geolocator.reverse(f"{str(lat)}, {str(lon)}", addressdetails=True, timeout=30)
    if location is None:
        return np.nan
...
def get_lat_lon(suburb: str, province: str) -> Tuple[float, float]:
    location = geolocator.geocode(f"{suburb}, {province}", addressdetails=True, timeout=30)
    if location is None:
